...
public static class CalculatePingDisplay
{
...
internal static void UpdateClient()
{
    if (Time.time - lastPingTime >= PingFrequency)
    {
        ...
        ClientSend.PlayerLocalTime(LocalTime());
        //ClientSend.PlayerLocalTime(Time.time);
...
internal static void OnClientPong(double _ping, double _pong)
{
    //double curr = Time.time;
    //myRtt = ((curr - _ping) - (2 *index))/2;

    double now = LocalTime();

    // how long did this message take to come back
    double newRtt = now - _ping;
    _rtt.Add(newRtt);

    myRtt = (newRtt - (2 * index))/2;
    ...
    double newOffset = now - newRtt * 0.5f - _pong;

    double newOffsetMin = now - newRtt - _pong;
    double newOffsetMax = now - _pong;
    ...
    //Debug.Log("RTT" + Math.Round(CalculatePingDisplay.rtt * 1000) + " ms");

    Debug.Log("MyRTT" + Math.Round(CalculatePingDisplay.myRtt * 1000) + " ms");
    UIManager.instance.updateRtt(Math.Round(CalculatePingDisplay.myRtt * 1000).ToString());
    //Debug.Log("NewRTT" + Math.Round(newRtt * 1000) + " ms");
}
