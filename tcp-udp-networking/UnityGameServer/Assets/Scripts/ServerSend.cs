...
public class ServerSend
{
    ...
    public static void PlayerShootAt(Player _player, Vector3 _pos, Vector3 _dir, string _hit_obj, int _hit_id)
    {
        using (Packet _packet = new Packet((int)ServerPackets.playerShootAt))
        {
            _packet.Write(_player.id);
            _packet.Write(_pos);
            _packet.Write(_dir);
            _packet.Write(_hit_obj);
            _packet.Write(_hit_id);
    ...
    public static void EnemyShootAt(int _enemy_id, Vector3 _pos, Vector3 _dir, string _hit_obj, int _hit_id)
    {
        using (Packet _packet = new Packet((int)ServerPackets.enemyShootAt))
        {
            _packet.Write(_enemy_id);
            _packet.Write(_pos);
            _packet.Write(_dir);
            _packet.Write(_hit_obj);
            _packet.Write(_hit_id);
    ...
    public static void Pong(int _playerID)
    {
    ...
    public static void GameEnd(int _numAllKilled)
    {
        using (Packet _packet = new Packet((int)ServerPackets.gameEnd))
        {
            _packet.Write(_numAllKilled);
            SendTCPDataToAll(_packet);
        }
    }
