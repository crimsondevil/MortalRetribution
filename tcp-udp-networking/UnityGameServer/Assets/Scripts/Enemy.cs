...
public class Enemy : MonoBehaviour
{
    ...
    private bool LookForPlayer()
    {
        foreach (Client _client in Server.clients.Values)
        {
            if (_client.player != null)
            {
                ...
                //Vector3 _enemyToPlayer = _client.player.transform.position - shootOrigin.position;
    ...
    private void Chase()
    {
        if (CanSeeTarget())
        {
            //Insert code here to serversend enemy seeing player
        ...
        if (Physics.Raycast(shootOrigin.position, _shootDirection, out RaycastHit _hit, shootRange))
        {
            if (_hit.collider.CompareTag("Player"))
            {
                if (Random.value <= shootAccuracy)
                {
                    ...
                    ServerSend.EnemyShootAt(this.id, shootOrigin.position, _shootDirection, "Player", _hit.collider.GetComponent<Player>().id);
                }
            }
        }
    }
